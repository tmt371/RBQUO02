...
; (() => {
  class DataTable {
    ...
    handleTableKeyDown(e) {
      if (e.target.cellIndex > 0) {
        if (e.key === "Enter") {
          ...
          if (
            currentRowIndex === this.table.rows.length - 1 &&
    ...
    updatePrices() {
      ...
      rows.forEach((row) => {
        const width = Number.parseFloat(row.cells[1].textContent)
        const height = Number.parseFloat(row.cells[2].textContent)
  ...
  const memoFindPrice = (() => {
    ...
    return (width, height, table) => {
      ...
      const isWilsonTable = table === priceDatabase.wilsonTable
      const widthRanges = isWilsonTable ? priceDatabase.wilsonWidthRanges : priceDatabase.sunsetUniviewWidthRanges
      const widthResult = priceDatabase.findNextRange(width, widthRanges)
  ...
  const updateTargetCell = (value) => {
    if (dataTable.activeCell) {
      ...
      if (value === "" || isValidInput(value, minValue, maxValue)) {
        ...
        if (
          currentRowIndex === dataTable.table.rows.length - 1 &&
          isValidInput(wCell.textContent) &&
          isValidInput(hCell.textContent)
        ) {
    ...
    button.addEventListener("click", () => {
      ...
      if (value === "Reset") {
        ...
        if (typeof dataTable !== "undefined" && dataTable !== null) {
          window.finishAndSubmit(dataTable)
        } else {
  ...
  const setDataTable = window.setDataTable
